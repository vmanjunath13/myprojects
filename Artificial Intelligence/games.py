...
def minimax_decision(state, game, calculate=False):
    ...
    if (calculate):
        return argmax(game.actions(state),
                  key=lambda a: min_value(game.result(state, a))), count
    return argmax(game.actions(state), key=lambda a: min_value(game.result(state, a)))
...
def alphabeta_search(state, game, calculate=False):
    ...
    def play_game(self, *players):
        ...
        while True:
            for player in players:
                ...
                self.display(state)
                self.utility(state, "X")
                print()
                if self.terminal_test(state):
                    self.display(state)
                    return self.utility(state, self.to_move(self.initial))

    def continue_game(self, state, disp=False, *players):
        """Play an n-person, move-alternating game."""
        while True:
            for player in players:
                move = player(self, state)
                ...
                if(disp):
                    ...
                    print()
                if self.terminal_test(state):
                    return self.utility(state, self.to_move(self.initial)), state
