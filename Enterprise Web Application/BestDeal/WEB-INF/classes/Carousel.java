			...
			if(user.equals(utility.username()))
			{
				...
				*/
				...
				for(String prod : productsList){
					...
					//System.out.println(prodObj.getProductType()+" "+prodObj.getproductImage());
					sb.append("<li id='item'><img src='images/"+prodObj.getProductType().toLowerCase().replaceAll("\\s+","")+"/"+prodObj.getproductImage()+"' alt='' '/></li>");
					sb.append("<li><form method='post' action='Cart'>" +
							"<input type='hidden' name='name' value='"+prod.trim()+"'>"+
							"<input type='hidden' name='type' value='"+prodObj.getProductType().toLowerCase().replaceAll("\\s+","")+"'>"+
